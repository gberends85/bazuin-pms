...
export async function generateInvoiceHtml(token: string): Promise<string | null> {
  ...
  let serviceRows = '';
  if (parseFloat(r.season_surcharge_amount || 0) > 0) {
  ...
  for (const v of vehicles) {
    if (v.ev_kwh || v.ev_service_id) {
      serviceRows += `<tr><td>Auto laden${v.license_plate ? ` — ${esc(v.license_plate)}` : ''}${v.ev_kwh ? ` (${v.ev_kwh} kWh)` : ''}</td><td class="num">1×</td><td class="num">${fmtMoney(v.ev_price || 0)}</td></tr>`;
    }
  }
  if (parseFloat(r.on_site_surcharge || 0) > 0) {
...
<div class="page">
  ...
  <table class="items-table">
    ...
    <tbody>
      <tr>
        ...
        <td class="num">${fmtMoney(r.base_price)}</td>
      </tr>
      ${serviceRows}
  ...
  <div class="footer">
    Autostalling De Bazuin · Zeilmakersstraat 2, 8861SE Harlingen · KVK: 51258692 · BTW: NL001451863B79<br>
