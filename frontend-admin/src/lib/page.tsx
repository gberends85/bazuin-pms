                ...
                <div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14, padding: '8px 10px', background: '#f4f6f9', borderRadius: 7 }}>
                    ...
                    >
                      ...
                      {ferryDoeksenSyncing ? 'Ophalen…' : '🔄 Ophalen van Doeksen'}
                    </button>
                  </div>
                  {/* Eilandkeuze */}
                  {(['outbound', 'return'] as const).map(dir => {
                    ...
                    const setDest = isOut
                      ? (d: 'terschelling'|'vlieland') => { setFerryOutDest(d); setFerryOutTime(''); loadFerrySchedules(res.arrival_date?.slice(0,10), res.departure_date?.slice(0,10), d, ferryRetDest); }
                      : (d: 'terschelling'|'vlieland') => { setFerryRetDest(d); setFerryRetTime(''); loadFerrySchedules(res.arrival_date?.slice(0,10), res.departure_date?.slice(0,10), ferryOutDest, d); };
                    ...
                    return (
                      <div key={dir} style={{ marginBottom: 16 }}>
                          ...
                          <>
                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 6 }}>
                              {schedules.map((s: any) => {
                                ...
                                const sel = time === t;
                                return (
                                  <button key={t} onClick={() => setTime(t)} style={{ padding: '6px 12px', borderRadius: 7, cursor: 'pointer', border: sel ? '2px solid #0a7c6e' : '0.5px solid rgba(10,34,64,0.2)', background: sel ? '#e6f7f5' : 'white', fontWeight: sel ? 800 : 600, color: sel ? '#0a7c6e' : '#0a2240', fontSize: 14 }}>
                                    {t}{s.isFast ? <span style={{ fontSize: 9, color: '#0a7c6e', display: 'block', fontWeight: 700 }}>SNEL</span> : null}
                                  </button>
                                );
                              })}
                            </div>
                        ...
                        {time && <div style={{ marginTop: 6, fontSize: 12, color: '#0a7c6e', fontWeight: 700 }}>✓ {time}</div>}
                      </div>
                    );
                  })}
