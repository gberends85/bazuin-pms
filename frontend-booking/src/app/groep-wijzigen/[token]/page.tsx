    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(10,34,64,0.45)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' }}
      ...
      <div style={{ background: 'white', borderRadius: 16, width: '100%', maxWidth: 500, maxHeight: '90vh', overflowY: 'auto', padding: '20px 18px 28px' }}>
        ...
        {(res.vehicles || []).map((v: any) => {
          ...
          // Aanbevolen tier = kleinste optie >= suggestedKwh (BEV: ~50% cap, PHEV: ~100% cap)
...
// ── ReservationCard ───────────────────────────────────────────────────────────
function ReservationCard({ res, evServices, groupToken, onSaved, showCopyLink = false }: {
  res: any; evServices: any[]; groupToken: string; onSaved: () => void; showCopyLink?: boolean;
}) {
  const [saving, setSaving] = useState(false);
  ...
  const [vehicleEvStatus, setVehicleEvStatus] = useState<Record<string, boolean | null>>({}); // null = loading, true = EV, false = not EV
    ...
    <>
      <div style={{ background: 'white', borderRadius: 14, border: '1px solid rgba(10,34,64,0.09)', marginBottom: 12, overflow: 'hidden', boxShadow: '0 1px 4px rgba(10,34,64,0.07)' }}>
        ...
        <div style={{ padding: '14px' }}>
          ...
          {/* Naam + telefoon — 3 kolommen op desktop, stapel op mobiel */}
          ...
          {/* Kentekens + Opladen-knop op dezelfde rij */}
          ...
          {/* Persoonlijke link kopiëren */}
          {showCopyLink && (
            <div style={{ paddingTop: 12, borderTop: '1px solid rgba(10,34,64,0.07)' }}>
              <button
                disabled={linkLoading || linkCopied}
                onClick={async () => {
                  const fullUrl = `${window.location.origin}/boeken/groep-wijzigen/${groupToken}/${res.id}`;
                  setLinkLoading(true);
                  try {
                    const apiUrl = process.env.NEXT_PUBLIC_API_URL || '/api/v1';
                    const r = await fetch(`${apiUrl}/public/short-link`, {
                      method: 'POST',
                      headers: { 'Content-Type': 'application/json' },
                      body: JSON.stringify({ url: fullUrl }),
                    });
                    const { code } = r.ok ? await r.json() : {};
                    const shortUrl = code
                      ? `${window.location.origin}/boeken/r/${code}`
                      : fullUrl;
                    await navigator.clipboard.writeText(shortUrl).catch(() => { window.prompt('Kopieer de persoonlijke link:', shortUrl); });
                    setLinkCopied(true);
                    setTimeout(() => setLinkCopied(false), 2500);
                  } catch {
                    await navigator.clipboard.writeText(fullUrl).catch(() => { window.prompt('Kopieer de persoonlijke link:', fullUrl); });
                    setLinkCopied(true);
                    setTimeout(() => setLinkCopied(false), 2500);
                  } finally {
                ...
                style={{ fontSize: 12, padding: '6px 12px', borderRadius: 8, border: '1px solid rgba(10,34,64,0.15)', background: linkCopied ? '#eaf1fb' : 'white', color: linkCopied ? '#19499e' : linkLoading ? '#9aafbf' : '#556070', cursor: linkLoading || linkCopied ? 'default' : 'pointer', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 5, transition: 'all 0.15s' }}
              >
                {linkCopied ? <><CheckIcon style={{ width: 13, height: 13 }} />Link gekopieerd!</> : linkLoading ? 'Link maken…' : '🔗 Persoonlijke link kopiëren'}
              </button>
              <div style={{ fontSize: 11, color: '#9aafbf', marginTop: 5 }}>
                Stuur deze link naar {firstName || res.first_name} — hij/zij ziet dan alleen zijn/haar eigen gegevens.
              </div>
            </div>
          )}
        </div>
...
// ── Main page ─────────────────────────────────────────────────────────────────
export default function GroepWijzigenPage({ params }: { params: { token: string } }) {
    ...
    <div style={{ minHeight: '100vh', background: '#f0f2f5', padding: '20px 16px 48px' }}>
      <div style={{ maxWidth: 520, margin: '0 auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
          ...
          <div>
            <div style={{ fontSize: 16, fontWeight: 800, color: '#142440', lineHeight: 1.2 }}>Groepsboeking wijzigen</div>
            <div style={{ fontSize: 12, color: '#9aafbf', marginTop: 1 }}>Autostalling De Bazuin · {group?.reference}</div>
          </div>
        </div>

        {reservations.length === 0
          ? <div style={{ background: 'white', borderRadius: 12, padding: '24px', textAlign: 'center', color: '#7090b0', fontSize: 14 }}>Geen boekingen gevonden.</div>
          : reservations.map((res: any) => <ReservationCard key={res.id} res={res} evServices={evServices} groupToken={params.token} onSaved={load} showCopyLink={true} />)
        }
